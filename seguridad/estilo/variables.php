...
$elemento_titulo="de estilo"; // para el título
$elemento="estilo"; // elemento o nomenclador al que se accede
...
$intro_ayuda="Esta p&aacute;gina muestra una ayuda respecto a los estilos registrados en la base de datos.";//introduccion de la ayuda

//------------------------------------------------LISTAR------------------------------------------------
//------------------------------------------------LISTAR------------------------------------------------
//------------------------------------------------LISTAR------------------------------------------------

$tabla="n_estilo";// tabla de la BD

$tabla_anidada=array();
//$tabla_anidada[0]='usuario';

//$tabla_anidada2[0]='estilo_rol';
//$tabla_anidada2[1]='n_rol';

$campo_anidado=array();
//$campo_anidado[0]='id_usuario';

//$where=" AND (estilo.id_estilo=estilo_rol.id_estilo or estilo.id_estilo!=estilo_rol.id_estilo) AND estilo_rol.id_rol=n_rol.id_rol";

$field[0]=$tabla.'.id_estilo'; // fields de la tabla en la BD, $field[0] es la llave de la tabla
$field[1]='estilo';
$field[2]='descripcion';

$alias_col=array();
$alias_col[0]='id_estilo';//alias de los campos de la consulta, debe haber alias en todos los campos
$alias_col[1]='estilo';
$alias_col[2]='descripcion';

$columna[0]=''; // labels o encabezados, $field[0] es la llave de la tabla por tanto no lleva label (NO TOCAR)
$columna[1]='Nombre de estilo';
...
$info_emerg[1]=1; // puede ser 1 o 2, al tener un valor diferente de vacío se muestra, si es 1 saca el valor de lo que muestra de la BD según la consulta, si es 2 saca el valor de los option estáticos
$info_emerg[2]=1;

$info_col[0]=''; // si se muestra o no la columna para listar
$info_col[1]=1; // puede ser 1 o 2, al tener un valor diferente de vacío se muestra, si es 1 saca el valor de lo que muestra de la BD según la consulta, si es 2 saca el valor de los option estáticos
$info_col[2]=1;
...
$var_mod='';
$columna_suma[0]='';
$href_m=$x.$camino.'/mod_estilo.php?mod='; // camino de la pagina para Editar
...
$l_label_ayuda[0]=array('texto'=>$columna[1],'exp'=>'Nombre de estilo');
$l_label_ayuda[1]=array('texto'=>$columna[2],'exp'=>'Descripci&oacute;n de estilo');
...
$insert_field[0]='id_estilo'; // campos de la tabla en la BD de la consulta para insertar, Editar, $insert_field[0] es la llave de la tabla principal
$insert_field[1]='estilo';// hay un # de arreglo menos pues se inserta solo los campos de la tabla
...
//$combo_sql[$ff] = "select id_usuario as id_usuario, usuario as usuario from usuario ORDER BY usuario";

$opt_name[$ff][0]='usuario';//dato que va a mostrar al estilo ejem: Elemento 1, Elemento 2, etc. otro: Administrador, Vendedor, Contador, Invitado, etc. 
$opt_name[$ff][1]='';//Además puedes tener del campo de la tabla en caso de selectBD
$opt_name[$ff][2]='';
$opt_name[$ff][3]='';

$opt_value[$ff][0]='id_usuario';//value del option, campo de la tabla externa de la consulta del combo
$opt_value[$ff][1]='';
$opt_value[$ff][2]='';
$opt_value[$ff][3]='';

$opt_sel[$ff][0]='id_usuario';//selected del option, campo de la tabla externa de la consulta del combo, en caso que sea de forma estática se pone en uno solo "selected"
...
$opt_name[$t][0]='Activo';//dato que va a mostrar al estilo ejem: Elemento 1, Elemento 2, etc. otro: Administrador, Vendedor, Contador, Invitado, etc. 
$opt_name[$t][1]='Inactivo';//Además puedes tener del campo de la tabla en caso de selectBD

$opt_value[$t][0]='1';//value del option, campo de la tabla externa de la consulta del combo
$opt_value[$t][1]='0';

$opt_sel[$t][0]='selected';//selected del option, campo de la tabla externa de la consulta del combo, en caso que sea de forma estática se pone en uno solo "selected"
...
$n_label_ayuda[0]=array('texto'=>$columna[1],'exp'=>'Nombre de estilo');
$n_label_ayuda[1]=array('texto'=>$columna[2],'exp'=>'Descripci&oacute;n de estilo');
...
$m_label_ayuda[0]=array('texto'=>$columna[1],'exp'=>'Nombre de estilo');
$m_label_ayuda[1]=array('texto'=>$columna[2],'exp'=>'Descripci&oacute;n de estilo');
