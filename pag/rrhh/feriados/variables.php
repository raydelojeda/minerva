...
$elemento_titulo="de los d&iacute;as feriados"; // para el título
$elemento="feriados"; // elemento o nomenclador al que se accede
...
$intro_ayuda="Esta p&aacute;gina muestra una ayuda respecto a los d&iacute;as feriados registrados en la base de datos.";//introduccion de la ayuda

//------------------------------------------------LISTAR------------------------------------------------
//------------------------------------------------LISTAR------------------------------------------------
//------------------------------------------------LISTAR------------------------------------------------

$tabla="n_feriados";// tabla de la BD

$tabla_anidada=array();
$tabla_anidada[0]='n_eventos_feriado';

//$where=' AND empleado.id_persona=persona.id_persona';

$campo_anidado=array();
$campo_anidado[0]='id_eventos_feriado';


$field[0]=$tabla.'.id_feriados'; // fields de la tabla en la BD, $field[0] es la llave de la tabla
$field[1]='feriado';
$field[2]='recuperable';
$field[3]=$tabla.'.id_eventos_feriado';
$field[4]='eventos_feriado';

$alias_col=array();
$alias_col[0]='ID_feriados';//alias de los campos de la consulta, debe haber alias en todos los campos
$alias_col[1]='FERIADO';
$alias_col[2]='RECUPERABLE';
$alias_col[3]='ID_EVENTOS';
$alias_col[4]='EVENTOS';

$columna[0]=''; // labels o encabezados, $field[0] es la llave de la tabla por tanto no lleva label (NO TOCAR)
$columna[1]='Fecha del feriado';
$columna[2]='Recuperable';
$columna[3]='Evento';
$columna[4]='Evento';
...
$l_label_ayuda[0]=array('texto'=>$columna[1],'exp'=>'Fecha del feriado');
$l_label_ayuda[1]=array('texto'=>$columna[2],'exp'=>'Recuperable');
$l_label_ayuda[2]=array('texto'=>$columna[4],'exp'=>'Evento por el cual existe el feriado');
...
$insert_field[0]='id_feriados'; // campos de la tabla en la BD de la consulta para insertar, Editar, $insert_field[0] es la llave de la tabla principal
$insert_field[1]='feriado'; // hay un # de arreglo menos pues se inserta solo los campos de la tabla
$insert_field[2]='recuperable';
$insert_field[3]='id_eventos_feriado';
$insert_field[4]='';
...
$combo_sql[$e] = "SELECT id_eventos_feriado as ID_EVENTOS, eventos_feriado as EVENTOS_FERIADO FROM N_eventos_feriado";

$opt_name[$e][0]='EVENTOS_FERIADO';//dato que va a mostrar al usuario ejem: Elemento 1, Elemento 2, etc. otro: Administrador, Vendedor, Contador, Invitado, etc. //Además puedes tener del campo de la tabla en caso de selectBD
$opt_value[$e][0]='ID_EVENTOS';//value del option, campo de la tabla externa de la consulta del combo
$opt_sel[$e][0]='ID_EVENTOS';//selected del option, campo de la tabla externa de la consulta del combo, en caso que sea de forma estática se pone en uno solo "selected"// cuando sea selecBD en la primera fila se pone el alias del id de la tabla externa asociada al selecBD
...
$n_label_ayuda[0]=array('texto'=>$columna[1],'exp'=>'Fecha del feriado');
$n_label_ayuda[1]=array('texto'=>$columna[2],'exp'=>'Recuperable');
$n_label_ayuda[2]=array('texto'=>$columna[4],'exp'=>'Evento por el cual existe el feriado');
...
$m_label_ayuda[0]=array('texto'=>$columna[1],'exp'=>'Fecha del feriado');
$m_label_ayuda[1]=array('texto'=>$columna[2],'exp'=>'Recuperable');
$m_label_ayuda[2]=array('texto'=>$columna[4],'exp'=>'Evento por el cual existe el feriado');
