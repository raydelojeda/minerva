...
$hoy=date("Y-m-d");
$sql_des="SELECT DISTINCT compromiso.id_compromiso FROM compromiso, desempeno, n_periodo WHERE 1 AND desempeno.id_compromiso=compromiso.id_compromiso AND desempeno.id_periodo=n_periodo.id_periodo AND fecha_conclusion>='".$hoy."' AND compromiso.id_compromiso='".$mod."'";//print $sql_des;die();
$rs_des=$db->Execute($sql_des) or die($db->ErrorMsg());//print $rs_des;

if($rs_des->fields['id_compromiso']=='')
