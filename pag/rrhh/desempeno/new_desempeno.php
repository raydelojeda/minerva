...
(SELECT MAX(fecha_cargo) FROM persona,empleado,cargo_empleado,n_cargo,usuario,ingreso_salida WHERE persona.id_persona=empleado.id_persona AND persona.id_persona=usuario.id_persona AND empleado.id_empleado=cargo_empleado.id_empleado AND cargo_empleado.id_cargo=n_cargo.id_cargo AND empleado.id_empleado=ingreso_salida.id_empleado AND baja='0' AND usuario='".$_SESSION["user"]."' AND clave='".$_SESSION["md5_password"]."')";
//print $sql_ev;die();
...
AND NOT EXISTS (SELECT 1 FROM desempeno,n_periodo WHERE desempeno.id_empleado = empleado.id_empleado AND desempeno.id_periodo = n_periodo.id_periodo AND fecha_conclusion>='".$hoy."') 
AND EXISTS 	(SELECT 1 FROM n_periodo WHERE fecha_conclusion>='".$hoy."') ORDER BY primer_apellido,segundo_apellido,primer_nombre,segundo_nombre DESC";
