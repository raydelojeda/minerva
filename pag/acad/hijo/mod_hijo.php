...
//--------------LLENAR CAJAS PARA DATOS CLINICOS--------------
$sql_d="SELECT id_datos_clinicos FROM datos_clinicos WHERE id_estudiante='".$id_estudiante."'";//print $sql_d.'<br>';
$rs_d=$db->Execute($sql_d) or die($db->ErrorMsg());

if(!isset($rs_d->fields[0]))
{
	...
	$id_datos_clinicos=$rs->fields['myid'];
}
else
$id_datos_clinicos=$rs_d->fields['id_datos_clinicos'];

include($x."pag/acad/datos_clinicos/variables.php");
$s_rs_cli=$obj->Consulta_llenar_cajas($id_datos_clinicos,$insert_field,$tabla,$db,$columna,$insert_alias);
//--------------LLENAR CAJAS PARA DATOS CLINICOS--------------
...
if(isset($_POST[$inputs[0]['name_input']]))
{
	...
	include($x."pag/acad/datos_clinicos/variables.php");
	$mensaje=$obj->Consulta_update($field_unico,$tabla,$field,$insert_field,$insert_alias,$name_input,$label_input,$db,$id_datos_clinicos,$tipo_input,$value_input,$x);
	$obj->Imprimir_mensaje($mensaje);	
...
<div style='margin: 0 auto;width:98%;'>
	<div class="tab-pane" id="tabPane1">
		...
		<div class="tab-page" id="tabPage2">
			...
			$tipo_input[2]='';
			$inputs=$obj_var->Llenar_inputs($tipo_input,$name_input,$value_input,$columna,$label_input,$dato_permit,$onclic,$size,$field_col,$texto_input,$placeholder,$evento);// para llenar los input (NO TOCAR)
			$obj->Generar_inputs($inputs,$s_rs_cli,$opt_value,$opt_sel,$opt_name,$db,$combo_sql,$insert_alias,'ul-li',$visualizar);
			if(isset($_GET['mensaje']))
			$mensaje=$_GET['mensaje'];
			if(isset($return[0]))
			$mensaje=$return[0];
