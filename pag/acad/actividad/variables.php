...
$alias_col=array();
$alias_col[0]='id_actividad';//alias de los campos de la consulta, debe haber alias en todos los campos
$alias_col[1]='id_clase_ins';
$alias_col[2]='id_clase_ins';
$alias_col[3]='id_tipo_ins';
$alias_col[4]='id_tipo_ins';
$alias_col[5]='act_ins';
$alias_col[6]='des_ins';
$alias_col[7]='fec_ins';
...
$combo_sql[$g] = "select id_tipo_actividad as id_tipo_ins, concat(abv_tipo_actividad_examen,' - ',tipo_actividad_examen) as tipo_ins 
FROM n_tipo_actividad WHERE 1 AND id_asignatura='".$id_asignatura."' AND id_subperiodo_evaluativo='".$id_subperiodo_evaluativo."' ORDER BY n_tipo_actividad.orden";//print $combo_sql[$g];//

$opt_name[$g][0]='tipo_ins';//dato que va a mostrar al usuario ejem: Elemento 1, Elemento 2, etc. otro: Administrador, Vendedor, Contador, Invitado, etc. 
$opt_name[$g][1]='';//Además puedes tener del campo de la tabla en caso de selectBD

$opt_value[$g][0]='id_tipo_ins';//value del option, campo de la tabla externa de la consulta del combo
$opt_value[$g][1]='';

$opt_sel[$g][0]='id_tipo_ins';//selected del option, campo de la tabla externa de la consulta del combo, en caso que sea de forma estática se pone en uno solo "selected"
...
$onclic[2]='';
$onclic[3]='';
$onclic[4]='ejecutar_ajax("calificacion/tarea_detalles.php","id_tipo_ins", "tarea_detalles_ins","");';
$onclic[5]='';
$onclic[6]='';
...
$campos_origen[7]='sel_filtro_tip';

$campos_origen[8]='ini';//inicia la tarea
$campos_origen[9]='fin';//finaliza la tarea
$campos_origen[10]='opc';//opcional la tarea
$campos_origen[11]='hdn_txt_descrip';//opcional la tarea

$campos_origen_limpiar=array();
$campos_origen_limpiar[0]=$name_input[4];
$campos_origen_limpiar[1]=$name_input[5];
$campos_origen_limpiar[2]=$name_input[6];
$campos_origen_limpiar[3]=$name_input[7];

$mini_n_botones=array(); // botonera que va a tener en el encabezado (NO TOCAR)
$mini_n_botones[0]=array('target'=>'_self','href'=>'#','onclic'=>"if(document.getElementById('txt_descrip')){".$mini_onsubmit_sin_cerrar."} else {".$mini_onsubmit."} if(ok==0)return false;if(document.getElementById('txt_descrip'))document.frm.hdn_txt_descrip.value=encodeURIComponent(CKEDITOR.instances.txt_descrip.getData());ejecutar_ajax('calificacion/ins_".$elemento.".php','".implode(",", $campos_origen)."','contenido_grid_calificaciones');limpiar_campos(String('".implode(",", $campos_origen_limpiar)."').split(','));",'src'=>'../../../img/general/guardar_mini.png','nombre'=>'guardar','texto'=>'Guardar','accion'=>'Insertar');
$mini_n_botones[1]=array('target'=>'_self','href'=>'#close','onclic'=>'vaciar_tmp();mostrarArchivos();','src'=>'../../../img/general/cerrar_mini.png','nombre'=>'cancelar','texto'=>'Salir','accion'=>'');
