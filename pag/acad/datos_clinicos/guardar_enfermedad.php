...
{	
	...
	$sql="SELECT id_enfermedad_est FROM enfermedad_est WHERE id_enfermedad='".$id_enfermedad."' AND id_datos_clinicos='".$id_datos_clinicos."'";//print $sql_p;
	$rs=$db->Execute($sql) or die($db->ErrorMsg());//print $rs_p;
	
	if(!isset($rs->fields[0]))
	{
		$ins="INSERT INTO enfermedad_est (id_datos_clinicos, id_enfermedad, tuvo, tiene, descripcion) 
		VALUES ('".$id_datos_clinicos."','".$id_enfermedad."','".$tuvo."','".$tiene."','".$desc."')";//print $i_sql."<br>";
		$db->Execute($ins) or die($db->ErrorMsg());
	}
	else
	{
		$upd="UPDATE enfermedad_est SET tuvo='".$tuvo."',tiene='".$tiene."',descripcion='".$desc."'
		WHERE id_enfermedad='".$id_enfermedad."' AND id_datos_clinicos='".$id_datos_clinicos."'";//print $upd;// die();
