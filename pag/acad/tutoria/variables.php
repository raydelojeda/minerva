...
$elemento_titulo="de tutor&iacute;a"; // para el título
$elemento="clase"; // elemento o nomenclador al que se accede
...
$combo_sql[$v] = "SELECT DISTINCT empleado_academico.id_empleado_academico as id_tutor, concat(primer_apellido,' ',segundo_apellido,' ',primer_nombre,' ',segundo_nombre,' - ',identificacion) as tutor_academico 
FROM persona,empleado,ingreso_salida, empleado_academico WHERE 1 AND empleado_academico.id_empleado=empleado.id_empleado AND empleado.id_persona=persona.id_persona AND empleado.id_empleado=ingreso_salida.id_empleado AND baja='0' ORDER BY primer_apellido,segundo_apellido";

$opt_name[$v][0]='tutor_academico';//dato que va a mostrar al usuario ejem: Elemento 1, Elemento 2, etc. otro: Administrador, Vendedor, Contador, Invitado, etc. 
$opt_name[$v][1]='';//Además puedes tener del campo de la tabla en caso de selectBD

$opt_value[$v][0]='id_tutor';//value del option, campo de la tabla externa de la consulta del combo

$opt_sel[$v][0]='id_tutor';//selected del option, campo de la tabla externa de la consulta del combo, en caso que sea de forma estática se pone en uno solo "selected"
...
//$n_botones[0]=array('target'=>'_self','href'=>'lis_'.$elemento.'.php','onclic'=>'','src'=>$x.'img/general/cancelar.png','nombre'=>'cancelar','texto'=>'Salir','accion'=>'');
//$n_botones[]=array('target'=>'_blank','href'=>$x.'plantillas/ayu_insertar.php?var='.$camino.'/variables.php','onclic'=>'','src'=>$x.'img/general/ayuda.png','nombre'=>'ayuda','texto'=>'Ayuda','accion'=>'');
