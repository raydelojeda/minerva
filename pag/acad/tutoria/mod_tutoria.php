<?php
include("tutoria.php");
if(!isset($obj_tutoria))$obj_tutoria = new tutoria();// (NO TOCAR)

include("../cuaderno_clase/cuaderno_clase.php");
			...
			<div class="tab-pane" id="tabPane" style="padding-left:7px;width:99.5%">
				...
				for($s=0;$s<$rs_gra->RecordCount();$s++)
				{
					...
					if($seccion_academica_ant!=$seccion_academica)
					{
						...
						$sql_tut_sec="SELECT grado_paralelo_periodo.id_tutor
						...
						AND n_periodo_academico.activo='1' AND usuario='".$_SESSION['user']."'";//print $sql_tut;
						$rs_tut_sec=$db->Execute($sql_tut_sec) or die($db->ErrorMsg());
						
						if(isset($rs_tut_sec->fields['id_tutor']))
						{
					?>
							<div class="tab-page">
					...
					$sql_tut_gra="SELECT grado_paralelo_periodo.id_tutor
					FROM  n_periodo_academico,
					...
					AND n_periodo_academico.activo='1' AND usuario='".$_SESSION['user']."'";//print $sql_tut;
					$rs_tut_gra=$db->Execute($sql_tut_gra) or die($db->ErrorMsg());
				
					if(isset($rs_tut_gra->fields['id_tutor']))
					{
				?>
						<div class="tab-page">
							...
							<div class="tab-pane" id="tabPane_par<?php print $s;?>" style="padding-left:7px;width:99.5%">
								...
								for($p=0;$p<$rs_par->RecordCount();$p++)
								{
									...
									$sql_tut_par="SELECT grado_paralelo_periodo.id_tutor
									FROM  n_periodo_academico,
									empleado_academico, empleado, persona, usuario, grado_paralelo_periodo, n_grado_paralelo
									WHERE 1
									AND grado_paralelo_periodo.id_grado_paralelo=n_grado_paralelo.id_grado_paralelo
									...
									AND n_periodo_academico.activo='1' AND usuario='".$_SESSION['user']."'";//print $sql_tut;
									$rs_tut_par=$db->Execute($sql_tut_par) or die($db->ErrorMsg());
									
									if(isset($rs_tut_par->fields['id_tutor']))
									{
									?>
										<div class="tab-page">
													...
													<div style='display:table-row;'>			
														...
														<div style='display:table-cell;width:50%;text-align:center;'>
															...
															<div style='display:table;width:50%;margin-left:0;' id='contenido_grid_comportamiento_<?php print $id_gra_par;?>'>	
																...
																<?php
																	$obj_tutoria->contenido_comportamientos($db, '../', $rs_gra->fields['id_grado'], $rs_par->fields['id_paralelo']);										
																?>
												...
												<div style='display:table;width:100%;margin:0;'>
													<div style='display:table-row;'>			
														...
														<div style='display:table-cell;width:50%;text-align:center;'>
															...
															<div style='display:table;width:50%;margin-left:0;' id='contenido_grid_comportamiento_<?php print $id_gra_par;?>'>	
															<?php
															$sel_filtro_recom='s_1';
															$sel_filtro_est='';
															$obj_tutoria->filtro_recomendaciones($db, $rs_sub, $rs_gra->fields['id_grado'], $rs_par->fields['id_paralelo'], $sel_filtro_recom);
															?>				
				
					
																<div style='display:table;width:100%;margin-left:auto;margin-right:auto;' id='div_recomendaciones_<?php print $id_gra_par;?>'>				
																	<?php
																	$obj_tutoria->contenido_recomendaciones($db, '../', $rs_gra->fields['id_grado'], $rs_par->fields['id_paralelo'], $sel_filtro_recom, $sel_filtro_est);										
																	?>
																</div>
															</div>
