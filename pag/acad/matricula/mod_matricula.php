...
if(isset($_POST[$inputs[0]['name_input']]))
{
	...
	$upd="UPDATE cargo_empleado SET activo='". 1 ."' WHERE id_cargo_empleado='".$mod."'";//print $upd; die();
	$db->Execute($upd) or die($db->ErrorMsg());
	
	$sql_i="SELECT id_empleado, id_cargo FROM cargo_empleado WHERE id_cargo_empleado='".$mod."'";
	$rs_i=$db->Execute($sql_i) or die($db->ErrorMsg());
	
	$id_empleado=$rs_i->fields['id_empleado'];
	$id_cargo=$rs_i->fields['id_cargo'];
	
	$upd="UPDATE cargo_empleado SET activo='". 0 ."' WHERE id_empleado='".$id_empleado."' AND id_cargo_empleado!='".$mod."'";//print $upd; die();
	$db->Execute($upd) or die($db->ErrorMsg());
	
	$upd="UPDATE cargo_empleado SET activo='". 0 ."' WHERE id_cargo='".$id_cargo."' AND id_cargo_empleado!='".$mod."'";//print $upd; die();
	$db->Execute($upd) or die($db->ErrorMsg());
