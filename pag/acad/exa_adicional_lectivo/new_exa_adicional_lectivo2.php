...
$sql_left="SELECT id_examen_periodo_lec as id_left, concat(examen_lec,' - ',asignatura,' - ',abreviatura) as leeft
FROM n_asignatura, n_examen_periodo_lec, n_periodo_lectivo, n_conf_academica WHERE 1 AND n_examen_periodo_lec.id_asignatura=n_asignatura.id_asignatura 
	...
	for($left=0;$left<$rs_left->RecordCount();$left++)
	{
		...
		if(isset($_POST[$cbx]))
		{
			...
			if(isset($_POST['sel_sustituye_nota_'.$rs_top->fields['id_top']]) AND $sustituye_nota!='no')
			{
				...
				{$mensaje=$mensaje.'<br>Debe escoger si sustituye la nota del per&iacute;odo: '.$rs_top->fields['top'];$sustituye_nota='no';}
			}
			...
			if($orden=='ok' AND $publica_nota=='ok' AND $sustituye_nota='ok' AND $tipo=='ok')
			{
				$i_sql="INSERT INTO n_exa_adicional_lectivo(id_examen_adicional, id_tipo_examen, id_examen_periodo_lec, publica_nota, sustituye_nota, orden) 
...
<table align="center" width='100%'>
	...
	<tr class="encabezado">
		<td align="left" >Examen lectivo y asignatura </td><td align="right" >Nombre del examen adicional:</td>
