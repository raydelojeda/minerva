...
$sql_upd="UPDATE recibidos SET leido='1' WHERE id_recibidos='".$mod."'";
$rs_upd=$db->Execute($sql_upd) or die($db->ErrorMsg());
...
WHERE 1 AND recibidos.id_persona_remitente=persona.id_persona AND recibidos.id_notificacion=notificacion.id_notificacion
...
$remitente=$rs_m->fields['remitente'];
...
if(isset($rs_adj))
{
	if($rs_adj->fields['nombre_adjunto']!='')
	{
		...
		{
			$nombre_adjunto.='<br>'.'<a href="../../../archivos/esquelas/'.$_SESSION['user'].'/'.$id_notificacion.'/'.utf8_encode($rs_adj->fields['nombre_adjunto']).'"  download="'.utf8_encode($rs_adj->fields['nombre_adjunto']).'">'.'<img width="15px" src="../../../img/general/descargar.png"/>&nbsp;'.utf8_encode($rs_adj->fields['nombre_adjunto']).'</a>';
		$rs_adj->MoveNext();
		}
	}
}
		...
		<div class="tab-page" id="tabPage1" >
			...
			<div style='display:table;width:100%;'>
				...
				<div style='display:table-row;'>					
					<div style='display:table-cell;width:100%;text-align:left;padding-left:1%;font-size:14px;'><?php echo '<img style="margin:3px;border-radius:5px;" width="50px" src="'.$x.'img/general/no_disponible.png"/>';print 'De: '.$remitente.' - '.$fecha_envio.' ';if($urgente==1)print '<img style="margin:3px;border-radius:5px;" width="16px" src="'.$x.'img/general/importante.png"/>';?></div>
				</div>
				<br>
				<div style='display:table-row;'>					
