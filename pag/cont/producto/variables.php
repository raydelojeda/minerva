...
$elemento_titulo="de producto"; // para el título
$elemento="producto"; // elemento o nomenclador al que se accede
...
$intro_ayuda="Esta p&aacute;gina muestra una ayuda respecto a los tiempos de la jornada extra registrados en la base de datos.";//introduccion de la ayuda

//------------------------------------------------LISTAR------------------------------------------------
//------------------------------------------------LISTAR------------------------------------------------
//------------------------------------------------LISTAR------------------------------------------------

$tabla="n_producto";// tabla de la BD

$tabla_anidada=array();
//$tabla_anidada[0]='n_producto';
//$tabla_anidada[1]='';

$campo_anidado=array();
//$campo_anidado[0]='id_producto';
//$campo_anidado[1]='';

$field[0]='id_producto'; // fields de la tabla en la BD, $field[0] es la llave de la tabla
$field[1]='cod_producto';
$field[2]='producto';

$alias_col=array();
$alias_col[0]='id_producto';//alias de los campos de la consulta, debe haber alias en todos los campos
$alias_col[1]='sel_cod';
$alias_col[2]='sel_pro';

$columna[0]=''; // labels o encabezados, $field[0] es la llave de la tabla por tanto no lleva label (NO TOCAR)
$columna[1]='C&oacute;digo del producto';
$columna[2]='Producto';
...
$l_label_ayuda[0]=array('texto'=>$columna[1],'exp'=>'C&oacute;digo del producto');
$l_label_ayuda[1]=array('texto'=>$columna[1],'exp'=>'Producto');
...
$insert_field[0]='id_producto'; // campos de la tabla en la BD de la consulta para insertar, Editar, $insert_field[0] es la llave de la tabla principal
$insert_field[1]='cod_producto'; // hay un # de arreglo menos pues se inserta solo los campos de la tabla
$insert_field[2]='producto';
...
$ff=5;
$f=6;
$combo_sql[$f] = "select id_producto as id_emp, nombre_emp as emp from n_producto order by fecha_crea";
...
$mini_n_botones[0]=array('target'=>'_self','href'=>'#','onclic'=>$mini_onsubmit."if(ok==0)return false;guardar_sin_recargar('".$x.$camino."/ins_".$elemento.".php','".implode(",", $campos_origen)."','pro');",'src'=>$x.'img/general/guardar_mini.png','nombre'=>'guardar','texto'=>'Guardar','accion'=>'Insertar');
...
$n_label_ayuda[0]=array('texto'=>$columna[1],'exp'=>'C&oacute;digo del producto');
$n_label_ayuda[1]=array('texto'=>$columna[1],'exp'=>'Producto');
...
$m_label_ayuda[0]=array('texto'=>$columna[1],'exp'=>'C&oacute;digo del producto');
$m_label_ayuda[1]=array('texto'=>$columna[1],'exp'=>'Producto');
