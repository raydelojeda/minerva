...
$elemento_titulo="de la referencia para la factura"; // para el título
$elemento="referencia"; // elemento o nomenclador al que se accede
...
$intro_ayuda="Esta p&aacute;gina muestra una ayuda respecto a las referencias para la factura registrados seg&uacute;n el cargo en la base de datos.";//introduccion de la ayuda

//------------------------------------------------LISTAR------------------------------------------------
//------------------------------------------------LISTAR------------------------------------------------
//------------------------------------------------LISTAR------------------------------------------------

$tabla="referencia";// tabla de la BD
...
$where=' AND cliente.id_persona=persona.id_persona';

$field[0]='id_referencia'; // fields de la tabla en la BD, $field[0] es la llave de la tabla
$field[1]=$tabla.'.id_punto_venta';
$field[2]='punto_venta';
$field[3]=$tabla.'.id_cliente';
$field[4]="concat(primer_apellido,' ',segundo_apellido,' ',primer_nombre,' ',segundo_nombre,' - ',identificacion)";
$field[5]='referencia';

$alias_col=array();
$alias_col[0]='id_referencia';//alias de los campos de la consulta, debe haber alias en todos los campos
$alias_col[1]='id_pun';
$alias_col[2]='pun';
$alias_col[3]='id_c';
$alias_col[4]='per';
...
$var_mod='';
$columna_suma[0]='';
$href_m='mod_referencia.php?mod='; // camino de la pagina para Editar
...
$l_label_ayuda[0]=array('texto'=>$columna[2],'exp'=>'Punto de venta asociado a la referencia');
$l_label_ayuda[1]=array('texto'=>$columna[4],'exp'=>'Cliente con una referencia en el punto de venta');
$l_label_ayuda[1]=array('texto'=>$columna[5],'exp'=>'Referencia');
...
$insert_field[0]='id_referencia'; // campos de la tabla en la BD de la consulta para insertar, Editar, $insert_field[0] es la llave de la tabla principal
...
$f=2;
$ff=4;
$combo_sql[$f] = "select id_punto_venta as id_pun, punto_venta as pun from n_punto_venta";
...
$opt_sel=array();
$opt_sel[$f][0]='id_pun';//selected del option, campo de la tabla externa de la consulta del combo, en caso que sea de forma estática se pone en uno solo "selected"
$opt_sel[$f][1]='';// cuando sea selecBD en la primera fila se pone el alias del id de la tabla externa asociada al selecBD
$opt_sel[$f][2]='';
$opt_sel[$f][3]='';

$combo_sql[$ff] = "SELECT id_cliente AS id_c, concat(primer_apellido,' ',segundo_apellido,' ',primer_nombre,' ',segundo_nombre,' - ',identificacion) as per FROM persona,cliente WHERE persona.id_persona=cliente.id_persona";

$opt_name[$ff][0]='per';//dato que va a mostrar al usuario ejem: Elemento 1, Elemento 2, etc. otro: Administrador, Vendedor, Contador, Invitado, etc. 
$opt_name[$ff][1]='';//Además puedes tener del campo de la tabla en caso de selectBD
$opt_name[$ff][2]='';
$opt_name[$ff][3]='';


$opt_value[$ff][0]='id_c';//value del option, campo de la tabla externa de la consulta del combo
$opt_value[$ff][1]='';
$opt_value[$ff][2]='';
$opt_value[$ff][3]='';


$opt_sel[$ff][0]='id_c';//selected del option, campo de la tabla externa de la consulta del combo, en caso que sea de forma estática se pone en uno solo "selected"
...
$n_label_ayuda[0]=array('texto'=>$columna[2],'exp'=>'Punto de venta asociado a la referencia');
$n_label_ayuda[1]=array('texto'=>$columna[4],'exp'=>'Cliente con una referencia en el punto de venta');
$n_label_ayuda[1]=array('texto'=>$columna[5],'exp'=>'Referencia');
...
$m_label_ayuda[0]=array('texto'=>$columna[2],'exp'=>'Punto de venta asociado a la referencia');
$m_label_ayuda[1]=array('texto'=>$columna[4],'exp'=>'Cliente con una referencia en el punto de venta');
$m_label_ayuda[1]=array('texto'=>$columna[5],'exp'=>'Referencia');
