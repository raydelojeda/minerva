...
$elemento_titulo="de grupo de nivel 3"; // para el título
$elemento="subgrupo"; // elemento o nomenclador al que se accede
...
$intro_ayuda="Esta p&aacute;gina muestra una ayuda respecto a los grupos de nivel 3 registrados en la base de datos.";//introduccion de la ayuda

//------------------------------------------------LISTAR------------------------------------------------
//------------------------------------------------LISTAR------------------------------------------------
//------------------------------------------------LISTAR------------------------------------------------

$tabla="n_subgrupo";// tabla de la BD

$tabla_anidada=array();
$tabla_anidada[0]='n_gru';


$campo_anidado=array();
$campo_anidado[0]='id_gru';


$field[0]='id_subgrupo'; // fields de la tabla en la BD, $field[0] es la llave de la tabla
$field[1]=$tabla_anidada[0].'.id_gru';
$field[2]='gru';
$field[3]='subgrupo';

$alias_col=array();
$alias_col[0]='id_subgrupo';//alias de los campos de la consulta, debe haber alias en todos los campos
$alias_col[1]='id_gru';
$alias_col[2]='sel_grupo';
$alias_col[3]='subgrupo';

$columna[0]=''; // labels o encabezados, $field[0] es la llave de la tabla por tanto no lleva label (NO TOCAR)
$columna[1]='Grupo de nivel 2';
$columna[2]='Grupo de nivel 2';
$columna[3]='Grupo de nivel 3';
...
$var_mod='';
$columna_suma[0]='';
$href_m='mod_subgrupo.php?mod='; // camino de la pagina para Editar
...
$l_label_ayuda[0]=array('texto'=>$columna[1],'exp'=>'Grupo de nivel 3');
$l_label_ayuda[1]=array('texto'=>$columna[3],'exp'=>'Grupo de nivel 2');
...
$l_botones[1]=array('target'=>'_self','href'=>'#','onclic'=>'modif(\'mod_subgrupo.php\')','src'=>$x.'img/general/Editar.png','nombre'=>'editar','texto'=>'Editar','accion'=>'Editar');
...
$insert_field[0]='id_subgrupo'; // campos de la tabla en la BD de la consulta para insertar, Editar, $insert_field[0] es la llave de la tabla principal
$insert_field[1]='id_gru'; // hay un # de arreglo menos pues se inserta solo los campos de la tabla
$insert_field[2]='';
$insert_field[3]='subgrupo';
...
$ff=5;
$f=2;
$combo_sql[$f] = "select id_gru as id_gru, gru as sel_grupo from n_gru order by gru";

$opt_name=array();
$opt_name[$f][0]='sel_grupo';//dato que va a mostrar al usuario ejem: Elemento 1, Elemento 2, etc. otro: Administrador, Vendedor, Contador, Invitado, etc. 
...
$opt_sel=array();
$opt_sel[$f][0]='id_gru';//selected del option, campo de la tabla externa de la consulta del combo, en caso que sea de forma estática se pone en uno solo "selected"
...
$campos_origen=array();
$campos_origen[0]=$alias_col[2];
$campos_origen[1]=$alias_col[3];

$mini_n_botones=array(); // botonera que va a tener en el encabezado (NO TOCAR)
$mini_n_botones[0]=array('target'=>'_self','href'=>'#','onclic'=>$mini_onsubmit."if(ok==0)return false;guardar_sin_recargar('".$x.$camino."/ins_".$elemento.".php','".implode(",", $campos_origen)."','sel_gru');",'src'=>$x.'img/general/guardar_mini.png','nombre'=>'guardar','texto'=>'Guardar','accion'=>'Insertar');
...
$n_label_ayuda[0]=array('texto'=>$columna[1],'exp'=>'Grupo de nivel 3');
$n_label_ayuda[1]=array('texto'=>$columna[3],'exp'=>'Grupo de nivel 2');
...
$m_label_ayuda[0]=array('texto'=>$columna[1],'exp'=>'Grupo de nivel 3');
$m_label_ayuda[1]=array('texto'=>$columna[3],'exp'=>'Grupo de nivel 2');
