...
$elemento_titulo="de consumibles disponibles en almac&eacute;n"; // para el título
$elemento="r_consumibles"; // elemento o nomenclador al que se accede
...
$select="SELECT id_inventario_consumibles, art_ext.id_articulo AS id_art, cod_articulo, concat(articulo,' - Marca: ',marca,' Modelo: ', modelo) AS art, SUM(inventario_consumibles.cantidad) AS cant_ent, (SELECT sum(pedido_consumible.cantidad) FROM pedido_consumible, n_articulo AS art_int WHERE art_ext.id_articulo=pedido_consumible.id_articulo AND art_int.cod_articulo=art_ext.cod_articulo ) AS cant_sale, (SUM(inventario_consumibles.cantidad)-(SELECT sum(pedido_consumible.cantidad) FROM pedido_consumible, n_articulo AS art_int WHERE art_ext.id_articulo=pedido_consumible.id_articulo AND art_int.cod_articulo=art_ext.cod_articulo)) AS resta FROM n_articulo AS art_ext, inventario_consumibles, n_marca, n_modelo WHERE art_ext.id_articulo=inventario_consumibles.id_articulo AND n_marca.id_marca=art_ext.id_marca AND n_modelo.id_modelo=art_ext.id_modelo GROUP BY cod_articulo";
...
$alias_col[1]='id_art';
$alias_col[2]='cod_articulo';
$alias_col[3]='art';
$alias_col[4]='$obj2->consumible_disponible($db,$rs->fields["id_art"])';

$columna=array();
$columna[0]=''; // labels o encabezados, $field[0] es la llave de la tabla por tanto no lleva label (NO TOCAR)
$columna[1]='C&oacute;d. art&iacute;culo';
$columna[2]='C&oacute;d. art&iacute;culo';
...
$l_label_ayuda[1]=array('texto'=>$columna[4],'exp'=>'Cantidad disponible');
...
$n_label_ayuda[1]=array('texto'=>$columna[4],'exp'=>'Cantidad disponible');
...
$m_label_ayuda[1]=array('texto'=>$columna[4],'exp'=>'Cantidad disponible');
