...
{ 
	...
	if(count($var) != 0)
	{
		for ($i = 0; $i < count($var); next($var), $i++) 
		{
			...
			$upd="UPDATE libro SET prestado='". 1 ."' WHERE id_libro='".$id_libro."'";//print $upd; die();
			$db->Execute($upd) or die($db->ErrorMsg());			
			
			$sql_p = "UPDATE prestamo SET  fecha_dev_real = '' WHERE id_prestamo='".$id."'";//print $sql_p;
			$db->Execute($sql_p) or die($db->ErrorMsg());
			
			if(!$mensaje)
			$mensaje='Devolución eliminada satisfactoriamente.';
		} 		
	} 
}
header("Location:lis_devolucion.php?mensaje=".utf8_encode($mensaje));/**/
?>
