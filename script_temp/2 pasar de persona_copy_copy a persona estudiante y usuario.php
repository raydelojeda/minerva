...
{ //print '# de persona_copy: '.$c.'<br>';
	...
	$id_grado_paralelo_periodo=$rs->fields["id_grado_paralelo_periodo"];
	...
	if(!isset($rs_p->fields["id_persona"]))
	{
		...
		if(!isset($rs_f->fields["id_persona"]))
		{
			...
			$id_estudiante=$rs_iu->fields["myid"];
			
			$sql_f="INSERT INTO curso_grado_paralelo_est (fecha_admision, fecha_matricula, codigo_matricula, fecha_retiro, cumplido, id_estudiante, id_grado_paralelo_periodo) 
			VALUES ('2015-09-01','2015-09-01','".$c."','0000-00-00','0','".$id_estudiante."', '".$id_grado_paralelo_periodo."')";
			$db->Execute($sql_f) or print $mensaje.'<br>'.$db->ErrorMsg();
