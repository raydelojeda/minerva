...
{ //print '# de persona_copy: '.$c.'<br>';
	...
	if(!isset($rs_p->fields["id_persona"]))
	{
		...
		$sql_f="SELECT id_persona FROM familiar where id_persona='".$id_persona."'";
		$rs_f=$db->Execute($sql_f) or print $mensaje.'<br>'.$db->ErrorMsg();
		
		
		if(!isset($rs_f->fields["id_persona"]))
		{
			$sql_f="INSERT INTO familiar (id_persona) VALUES ('".$id_persona."')";
			...
			if(!isset($rs_u->fields["id_persona"]))
			{
				...
				$sql_f="INSERT INTO usuario_rol (id_usuario, id_rol) VALUES ('".$id_usuario."','11')";
				$db->Execute($sql_f) or print $mensaje.'<br>'.$db->ErrorMsg();
			}
		}
	...
	elseif($identificacion!='')//es empleado
	{
		...
		if(isset($rs_pp->fields["id_persona"]))
		{
			...
			if(!isset($rs_f->fields["id_persona"]))
			{
				$sql_f="INSERT INTO familiar (id_persona, empresa_oficina, dir_trabajo, tel_trabajo, ocupacion) VALUES ('".$id_persona."','Unidad Educativa Atenas', ' Calle Gabriel Román s/n y Av. Pedro Vásconez Yacupamba, Izamba.Ambato.Ecuador EC 180156','(+593) 032854281','Docente')";
			...
			print 'ya estaba en fam: '.$identificacion.'<br>'; 
		}
	}
	else
